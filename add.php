...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $stmt = $conn->prepare("INSERT INTO employees (fullname, email, department, position, salary, date_hired) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("ssssds", $fullname, $email, $department, $position, $salary, $date_hired);
        if ($stmt->execute()) {
            header("Location: index.php?msg=added");
            exit();
        } else {
            $errors[] = "Error saving. Email may already exist.";
        }
        $stmt->close();
    }
}
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        input:focus, select:focus { border-color: #4c51bf; outline: none; }
        ...
        <div class="card-body">
            ...
            <form method="POST">
                ...
                <div class="form-group">
                    ...
                    <input type="number" step="0.01" name="salary" value="<?= htmlspecialchars($_POST['salary'] ?? '') ?>">
                </div>
                <div class="form-group">
                    <label>Date Hired *</label>
                    <input type="date" name="date_hired" value="<?= htmlspecialchars($_POST['date_hired'] ?? '') ?>">
                </div>
